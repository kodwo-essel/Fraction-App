...
/** Export only the allocation rules — safe to share publicly, no personal data. */
export const exportConfig = async (
    ...
    const fileName = `fraction-config-${dateStr}.json`;
    const fileUri = `${cacheDirectory}${fileName}`;
    ...
    await Sharing.shareAsync(fileUri, {
        mimeType: 'application/json',
        dialogTitle: 'Share your Fraction configuration',
        UTI: 'public.json',
    });
};

export type PickConfigResult =
    | { status: 'pending'; data: ConfigFile }
    | { status: 'cancelled' }
    | { status: 'invalid'; reason: string };

/** Open file picker and validate a config file. No DB writes. */
export const pickConfigFile = async (): Promise<PickConfigResult> => {
    const result = await DocumentPicker.getDocumentAsync({
        type: '*/*',
        copyToCacheDirectory: true,
    });
    ...
    let raw: string;
    try {
        raw = await readAsStringAsync(result.assets[0].uri, { encoding: EncodingType.UTF8 });
    } catch {
        return { status: 'invalid', reason: 'Could not read the selected file.' };
    }
    ...
    if (
        ...
        return { status: 'invalid', reason: 'This does not appear to be a Fraction configuration file.' };
    }

    return { status: 'pending', data: config };
};

/** Replace all categories with those from the config. Transactions untouched. */
